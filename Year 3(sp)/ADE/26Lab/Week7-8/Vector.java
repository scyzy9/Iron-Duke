/** COMP2054-ADE - LAB - FORMATIVE EXERCISE 
	...
	This is a reasonable, but not perfect implementation -- we are doing algorithms not programming style :-).
	
	TODO You should review the "C += 0" commands, and probably modify them to be what you think is better.
...
public class Vector {
	...
	public int count; // use for counting primitive operations - bad style to make it public, but is just for simplicity
	...
	//		System.out.print(A[i] + ",");
		//}
		//System.out.print("]\n");
		int r = A[m-1];  count +=  0; // rough value
		m--;           count += 0;
		return r;
	}
	...
	private void resize(int newSize) {
		...
		int[] newA = new int [newSize]; count+=newSize+10; // see lab exercises on primitive counting - and remember that Java initialises - though C/C++ do not
		for (int i = 0; i < A.length; i++) {
			newA[i] = A[i];count+=3;
		}
...
//		resize( A.length + 200 ); count += 2;  // increase by 200
		resize( 2 * A.length ); count += 2;    // double size
